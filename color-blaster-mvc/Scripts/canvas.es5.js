...
var Game = {
    ...
    load: function load() {
        ...
        for (var x = 0; x < this.level.columns; x++) {
    ...
    displayProgress: (function (_displayProgress) {
        function displayProgress() {
            return _displayProgress.apply(this, arguments);
        }

        displayProgress.toString = function () {
            return _displayProgress.toString();
        };
    ...
    loadLevel: function loadLevel() {
        ...
        if (!this.level.isSpecialLevel) {
            ...
            // if it is, do it differently
            var map = this.level.map;
            var blobIndex = 0; // NOTE if level setup is messed up, this can cause issues
            ...
            for (var y = 0; y < map.grid.length; y++) {
                for (var x = 0; x < map.grid[y].length; x++) {
                    // if it's a block
                    if (map.items[map.grid[y][x]] != null && map.items[map.grid[y][x]].includes('block')) {
                        var gridString = map.items[map.grid[y][x]];
                        var stringEnd = gridString.length;
                        stringEnd = stringEnd - 6;
                        var blockColor = gridString.substring(0, stringEnd);
                        // create block
                        var newBlock = new Block(this.tileWidth, this.tileHeight, x, y, blockColor);
                        // add block and put on canvas
                        this.blocks[x][y] = newBlock;
                        this.drawBlock(newBlock);
                    } else if (map.items[map.grid[y][x]] != null && map.items[map.grid[y][x]].includes('blob')) {
                        // create blob
                        var newBlob = new Blob(x, y, this.level.blobSpeeds[blobIndex], this.level.blobColors[blobIndex], this.level.blobDesiredColors[blobIndex], this.tileWidth, this.tileHeight);
                        // add blob
                        ...
                        blobIndex++;
                        // if it's an npc
                    }if (map.items[map.grid[y][x]] != null && map.items[map.grid[y][x]].includes('npc')) {
        ...
        this.inventory.setLevel(this.level);
    ...
    keyDownListener: function keyDownListener(event) {
        ...
        if (event.key === 'Enter') {
            ...
            } else if (instructionsModal.style.display != "none") {
                // this should only do something if a modal is displayed
    ...
    gameLoop: function gameLoop() {
        ...
        var timePassed = (newTimestamp - Game.timestamp) / 1000;
    ...
    generateBlobs: function generateBlobs() {
        // clear blobs
        this.blobs.length = 0;

        // generate all blobs for the level
        for (var i = 0; i < this.level.blobCount; i++) {
            this.generateBlob();
        }
    },
    ...
    generateBlob: function generateBlob() {
        ...
        var collidesWithCharacter = false;
        var newBlob = null;
        do {
            // get colors allowed for blob
            var possibleColors = getTileColors(this.level);
            ...
            do {
                ...
                if (this.level.blobsMustBeInBlocks) {
                    ...
                    var minYi = this.spaceAtEdges + 1;
                    var maxYi = rows - this.spaceAtEdges - 2;
                    do {
            ...
            var blobColorIndex = Math.floor(Math.random() * possibleColors.length);
            var blobColor = possibleColors[blobColorIndex];

            // determine desired color
            var desiredColorIndex = 0;
            do {
                desiredColorIndex = Math.floor(Math.random() * possibleColors.length);
            } while (desiredColorIndex === blobColorIndex);
            var desiredColor = possibleColors[desiredColorIndex];

            // determine speed
            var blobSpeed = this.level.maxBlobSpeed;
            if (this.level.randomizeBlobSpeed) {
                blobSpeed = Math.ceil(Math.random() * this.level.maxBlobSpeed);
            }

            // create the blob
            newBlob = new Blob(Xi, Yi, blobSpeed, blobColor, desiredColor, this.tileWidth, this.tileHeight);
    ...
    canMoveBlobInDirection: function canMoveBlobInDirection(blob, direction) {
        //console.log('checking blob direction: ' + direction);
        // get new coordinates for direction
        var testPosition = blob.getNewDirectionCoordinates(direction);
        var newX = testPosition[0];
        var newY = testPosition[1];

        // make sure it's a valid position - if not, return false
        var maxX = this.context.canvas.width - blob.spriteWidth + blob.rightEmptyPixels;
        var maxY = this.context.canvas.height - blob.spriteHeight;
        if (newX < 0 || newX >= maxX || newY < 0 || newY >= maxY) {
            return false;
        }

        // now check for block collision and npc collision
        var isBlockCollision = this.isBlockCollision('blob', newX, newY, blob);

        var newRelatives = this.getRelativePosition(newX, newY);

        var isNpcCollision = false;
        for (var i = 0; i < this.npcs.length; i++) {
            if (this.isBlobNpcCollision(this.npcs[i], newRelatives[0], newRelatives[1])) {
                isNpcCollision = true;
    ...
    moveBlobTowardExit: function moveBlobTowardExit(blob) {
        // don't allow character exit until blob is finished
        this.allowExit = false;

        // get exit position
        var exitX = this.door.x + this.tileWidth - blob.spriteWidth / 2;
        var exitY = this.context.canvas.height - 1;

        var halfExitX = undefined;
        if (exitX >= blob.positionBeforeExitX) {
            var halfAmount = Math.floor((exitX - blob.positionBeforeExitX) / 2);
            halfExitX = blob.positionBeforeExitX + halfAmount;
        } else {
            var halfAmount = Math.floor((blob.positionBeforeExitX - exitX) / 2);
            halfExitX = blob.positionBeforeExitX - halfAmount;
        }

        var halfExitY = undefined;
        if (exitY >= blob.positionBeforeExitY) {
            var halfAmount = Math.floor((exitY - blob.positionBeforeExitY) / 2);
            halfExitY = blob.positionBeforeExitY + halfAmount;
        } else {
            var halfAmount = Math.floor((blob.positionBeforeExitY - exitY) / 2);
            halfExitY = blob.positionBeforeExitY - halfAmount;
        }
        ...
        } else if (blob.x === halfExitX && blob.y != halfExitY) {
                // otherwise head toward y
                if (halfExitY >= blob.y) {
                    // move forward
    ...
    moveBlob: function moveBlob(blob) {
        // if the blob's direction is set to none but it is happy, then set the direction to 'exit'
        if (blob.direction === 'none' && blob.thoughts.currentThought === blob.thoughts.happyThought) {
            blob.direction = 'exit';
        }
    ...
    setBlobDirection: function setBlobDirection(blob) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            var randomIndex = this.getRandomDirectionIndex(directions);
            ...
            if (this.canMoveBlobInDirection(blob, randomDirection)) {
                ...
                // if not, remove direction from list of directions and try again
                directions.splice(randomIndex, 1);
            }
        }
        // now set the new direction for the blob
        var newCoordinates = blob.getNewDirectionCoordinates(newDirection);
        blob.setMoveDirection(newDirection, newCoordinates[0], newCoordinates[1]);
    },

    getRandomDirection: function getRandomDirection(directions) {
        var index = Math.floor(Math.random * directions.length);
        return directions[index];
    },

    getRandomDirectionIndex: function getRandomDirectionIndex(directions) {
        var index = Math.floor(Math.random() * directions.length);
        return index;
    },

    drawBlob: function drawBlob(blob) {
        var sx = blob.currentStateFrame.startX;
        var sy = blob.currentStateFrame.startY;

        var sw = blob.currentStateFrame.width;
        var sh = blob.currentStateFrame.height;

        var offX = Math.round((blob.currentStateFrame.width - this.tileWidth) / 2);
        var offY = Math.round((blob.currentStateFrame.height - this.tileHeight) / 2);
        ...
        if (dy < blob.yOffset) {
            // there is space at the top of the sprite so that's why it's negative
            dy = blob.yOffset;
        ...
        var image = blob.currentStateFrame.image;
    ...
    drawThought: function drawThought(blob, blobDx, blobDy) {
        var sx = blob.thoughts.currentThought.startX;
        var sy = blob.thoughts.currentThought.startY;

        var sw = blob.thoughts.currentThought.width;
        var sh = blob.thoughts.currentThought.height;

        var dx = blobDx + blob.thoughts.offX;
        var dy = blobDy + blob.thoughts.offY;
        ...
        var image = blob.thoughts.getThoughtImage();
        if (image != null) {
            // only do it if it's not null - meaning it's supposed to be shown
            var isLoaded = image.complete && image.naturalHeight !== 0;
            if (!isLoaded) {
                image.addEventListener('load', function () {
                    Game.context.drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh);
    ...
    drawCharacterThought: function drawCharacterThought(character, characterDx, characterDy) {
        ...
        if (character.thoughts.currentThought != null) {
            (function () {
                ...
                var sx = character.thoughts.currentThought.startX;
                var sy = character.thoughts.currentThought.startY;

                var sw = character.thoughts.currentThought.width;
                var sh = character.thoughts.currentThought.height;

                var dx = characterDx + character.thoughts.offX;
                var dy = characterDy + character.thoughts.offY;
                ...
                var image = character.thoughts.getThoughtImage();
                if (image != null) {
                    // only do it if it's not null - meaning it's supposed to be shown
                    var isLoaded = image.complete && image.naturalHeight !== 0;
                    if (!isLoaded) {
    ...
    drawBlobsAndCharacter: function drawBlobsAndCharacter() {
        // sort blobs into those behind and in front of character
        var behindBlobs = new Array();
        var inFrontBlobs = new Array();
        for (var i = 0; i < this.blobs.length; i++) {
        ...
        var behindNpcs = new Array();
        var inFrontNpcs = new Array();
        for (var i = 0; i < this.npcs.length; i++) {
            var npcY = undefined;
            var characterY = undefined;
    ...
    drawBlobs: function drawBlobs(blobs) {
    ...
    showInventory: function showInventory() {
        // grab the container
        var container = document.getElementById('inventoryDisplay');

        // clear it out but then put selected div inside
        container.innerHTML = '';

        // loop through vials and display them accordingly
        for (var i = 0; i < this.inventory.vials.length; i++) {
            var htmlString = "";
            ...
            if (this.inventory.vials[i] === this.inventory.activeVial) {
                htmlString += '<img id="vial' + i + '" class="vial selected" src="' + this.inventory.vials[i].getImageUrl() + '"><br>';
            } else {
                htmlString += '<img id="vial' + i + '" class="vial" src="' + this.inventory.vials[i].getImageUrl() + '"><br>';
            }
            container.innerHTML += htmlString;
        }
    },

    setShootOrAbsorb: function setShootOrAbsorb() {
    ...
    changeBlock: function changeBlock(bullet, block) {
    ...
    shootBullet: function shootBullet() {
        ...
        if (this.inventory.shootOrAbsorb === 'shoot' && this.inventory.activeVial.content != null) {
            var bullet = null;
            var radius = this.bulletRadius;
            var bulletX = 0;
            var bulletY = 0;
            var color = this.inventory.activeVial.content;

            var xSpeed = 0;
            var ySpeed = 0;

            var characterX = this.character.x + this.character.spriteWidth / 2;
            var characterY = this.character.y + this.character.spriteHeight / 2 + 13;
            ...
            bullet = new Bullet(radius, bulletX, bulletY, xSpeed, ySpeed, color);
    ...
    shootAbsorbRay: function shootAbsorbRay() {
        // *** using a bullet object to do this - but it will look like a ray

        // make sure player is able to absorb
        if (this.inventory.shootOrAbsorb === 'absorb' && this.inventory.activeVial.content === null) {
            this.absorbRay = null;
            var radius = this.bulletRadius;
            var rayX = 0;
            var rayY = 0;
            var color = this.inventory.absorbColors[this.inventory.activeAbsorbColorIndex];
            ...
            this.absorbRay = new Bullet(radius, rayX, rayY, xSpeed, ySpeed, color);
    ...
    moveBullets: function moveBullets() {
        ...
        for (var i = 0; i < this.bullets.length; i++) {
            ...
            var isBlockCollision = this.isBlockCollision('bullet', bullet.x, bullet.y, bullet);
            var isBlobCollision = this.isBulletBlobCollisionAll(bullet);
            console.log('blob bullet collision? ' + isBlobCollision);

            // check if the bullet is off the screen - remove if yes
            if (bullet.x < 0 || bullet.y < 0 || bullet.x > this.context.canvas.width || bullet.y > this.context.canvas.height) {
                indexesToRemove.push(i);
                // set the active vial back to the color it was
                this.inventory.activeVial.content = bullet.color;
                this.showInventory();
            } else if (isBlockCollision) {
                // now check for tile collision

                var relativePos = this.getRelativePosition(bullet.x, bullet.y);

                // check if the block color changes - if it doesn't,
                // set the active vial back to its normal content
                var block = this.blocks[relativePos[0]][relativePos[1]];

                // only do this if the block isn't null
                if (block != null) {
                    var originalBlockColor = block.color;
            ...
            } else if (isBlobCollision) {
                // if it collided with a blob
                console.log('bullet hit blob.');
                // get which blob is changing
                var blob = this.getBlobFromBulletCollision(bullet);

                var originalBlobColor = blob.color;
    ...
    changeBlob: function changeBlob(addOrSubtract, blob, bullet) {
    ...
    isBulletBlobCollisionAll: function isBulletBlobCollisionAll(bullet) {
        for (var i = 0; i < this.blobs.length; i++) {
            if (this.isBlobCollision('bullet', bullet.x, bullet.y, bullet, this.blobs[i])) {
                return true;
            }
        }
        return false;
    ...
    getBlobFromBulletCollision: function getBlobFromBulletCollision(bullet) {
        for (var i = 0; i < this.blobs.length; i++) {
            if (this.isBlobCollision('bullet', bullet.x, bullet.y, bullet, this.blobs[i])) {
                return this.blobs[i];
            }
    ...
    useRay: function useRay() {
        // do all the moving before drawing
        var ray = this.absorbRay;
        var rayColor = this.emptyAbsorbHex; // TODO determine hex color for it
        console.log('ray color before: ' + this.emptyAbsorbHex);
        var startX = ray.x; // these are needed for drawing the beam
        var startY = ray.y;

        // keep moving until there's a collision
        var isBlockCollision = undefined;
        var isBlobCollision = undefined;
        ...
        do {
            ...
            if (ray.x < 0 || ray.y < 0 || ray.x > this.context.canvas.width || ray.y > this.context.canvas.height) {
                isOverEdge = true;
            }
            ...
            if (isBlockCollision) {
                ...
                var relativePos = this.getRelativePosition(ray.x, ray.y);
                var block = this.blocks[relativePos[0]][relativePos[1]];
                if (block != null) {
                    var absorbResult = block.getSubtractResult(ray.color, block.color);
                    var newBlockColor = absorbResult[0];
                    ...
                    var rayWidth = this.rayWidth;
                    var rayHeight = this.rayWidth;
                    startX = startX - Math.round(rayWidth / 2);
                    startY = startY - Math.round(rayHeight / 2);
                    var newStartX = undefined;
                    var newStartY = undefined;
                    var endX = undefined;
                    var endY = undefined;
                    if (ray.x > startX) {
                        newStartX = startX;
                    ...
                    this.rayDisplay = new RayDisplay(startX, startY, rayWidth, rayHeight, rayColor, newBlockColor);
                ...
                isBlockCollision = false;
                var blob = this.getBlobFromBulletCollision(ray);
                if (blob != null) {
                    var absorbResult = blob.getSubtractResult(ray.color, blob.color);
                    var newBlobColor = absorbResult[0];
                    ...
                    this.rayDisplay = new RayDisplay(startX, startY, rayWidth, rayHeight, rayColor, newBlobColor);
    ...
    drawRay: function drawRay() {
        if (this.absorbRay != null && this.rayDisplay != null) {
            // draw the ray
            this.context.beginPath();
            this.context.fillStyle = this.rayDisplay.rayColor;

            this.context.fillRect(this.rayDisplay.startX, this.rayDisplay.startY, this.rayDisplay.rayWidth, this.rayDisplay.rayHeight);
            ...
            if (this.rayDisplay.rayColor === this.emptyAbsorbHex) {
                var image = this.confusedImage;
                var newX = this.character.x + this.character.spriteWidth / 2;

                var isLoaded = image.complete && image.naturalHeight !== 0;
                if (!isLoaded) {
                    this.context.drawImage(image, newX, this.character.y);
                } else {
                    this.context.drawImage(image, newX, this.character.y);
                }
            }
    ...
    drawBullets: function drawBullets() {
        ...
        for (var i = 0; i < this.bullets.length; i++) {
            ...
            // draw the bullet
            this.context.beginPath();
            this.context.arc(bullet.x, bullet.y, bullet.radius, 0, 2 * Math.PI, false);
    ...
    checkBlobCharacterCollision: function checkBlobCharacterCollision() {
        if (this.levelStarted) {
            ...
            for (var i = 0; i < this.blobs.length; i++) {
                ...
                if (isCollision && this.levelStarted === true) {
                    ...
                    if (this.blobs[i].thoughts.currentThought === this.blobs[i].thoughts.happyThought) {
                        isCollision = false;
                    } else {
                        break;
                    }
    ...
    showCharacterDeath: function showCharacterDeath() {
        document.getElementById('characterDeathModal').style.display = 'block';
    },

    isBlobCharacterCollision: function isBlobCharacterCollision(blob) {
        // store relevent blob values
        var blobLeftX = blob.x + blob.leftEmptyPixels + 1;
        var blobRightX = blob.x + blob.spriteWidth - blob.rightEmptyPixels - 1;
        var blobTopY = blob.y + blob.topEmptyPixels + 1;
        var blobBottomY = blob.y + blob.spriteHeight + 1;

        var characterLeftX = this.character.x - this.character.xOffset;
        var characterRightX = this.character.x + this.character.spriteWidth + this.character.xOffset;
        var characterTopY = this.character.y - this.character.yOffset + (this.character.spriteHeight / 2 + 4); // let it overlap if it's halfway
        var characterBottomY = this.character.y + this.character.spriteHeight;
        // the character bottom one doesn't need an offset, just the top

        if (characterRightX > blobLeftX && characterRightX < blobRightX || characterLeftX < blobRightX && characterLeftX > blobLeftX) {
            if (characterBottomY > blobTopY && characterBottomY < blobBottomY || characterTopY < blobBottomY && characterTopY > blobTopY) {
    ...
    isNpcCharacterCollision: function isNpcCharacterCollision(npc, newX, newY) {
        if (npc.species === "human") {
            // store relevent values
            var npcLeftX = npc.x - npc.xOffset;
            var npcRightX = npc.x + npc.spriteWidth + npc.xOffset;
            var npcTopY = npc.y - npc.yOffset + (npc.spriteHeight / 2 + 8); // let it overlap if it's halfway
            var npcBottomY = npc.y + npc.spriteHeight;

            var characterLeftX = newX - this.character.xOffset;
            var characterRightX = newX + this.character.spriteWidth + this.character.xOffset;
            var characterTopY = newY - this.character.yOffset + (this.character.spriteHeight / 2 + 8); // let it overlap if it's halfway
            var characterBottomY = newY + this.character.spriteHeight;
            // the character bottom one doesn't need an offset, just the top

            if (characterRightX > npcLeftX && characterRightX < npcRightX || characterLeftX < npcRightX && characterLeftX > npcLeftX) {
                if (characterBottomY >= npcTopY && characterBottomY <= npcBottomY || characterTopY <= npcBottomY && characterTopY >= npcTopY) {
    ...
    isBlobNpcCollision: function isBlobNpcCollision(npc, blobXi, blobYi) {
        if (npc.species === "human") {
            if (blobXi === npc.Xi && blobYi === npc.Yi || blobXi === npc.Xi && blobYi === npc.Yi - 1) {
                return true;
            }
        } else {
    ...
    isBlobCollision: function isBlobCollision(thingToCheck, itemX, itemY, item, blob) {
        // store relevent blob values
        var blobLeftX = blob.x + blob.leftEmptyPixels + 1;
        var blobRightX = blob.x + blob.spriteWidth - blob.rightEmptyPixels - 1;
        var blobTopY = blob.y + blob.topEmptyPixels + 1;
        ...
        if (thingToCheck === 'bullet') {
            ...
            var bulletLeftX = itemX - item.radius;
            var bulletRightX = itemX + item.radius;
            var bulletTopY = itemY - item.radius;
            var bulletBottomY = itemY + item.radius;

            if (bulletRightX >= blobLeftX && bulletRightX <= blobRightX || bulletLeftX <= blobRightX && bulletLeftX >= blobLeftX) {
                if (bulletBottomY >= blobTopY && bulletBottomY <= blobBottomY || bulletTopY <= blobBottomY && bulletTopY >= blobTopY) {
                    return true;
                }
            }
        } else if (thingToCheck === 'ray') {

            var bulletLeftX = itemX - 1;
            var bulletRightX = itemX + 1;
            var bulletTopY = itemY - 1;
            var bulletBottomY = itemY + 1;
    ...
    isBlockCollision: function isBlockCollision(thingToCheck, itemX, itemY, item) {
        ...
        for (var Xi = 0; Xi < this.level.columns; Xi++) {
            ...
            for (var Yi = 0; Yi < rows; Yi++) {
                ...
                if (block != null) {
                    ...
                    var blockLeftX = block.x * this.tileWidth;
                    var blockRightX = (block.x + 1) * this.tileWidth - 1;
                    var blockTopY = block.y * this.tileHeight;
                    var blockBottomY = (block.y + 1) * this.tileHeight - 1;

                    // check character collision - only bother if collision hasn't been found
                    if (thingToCheck === 'character') {
                        var characterLeftX = itemX - this.character.xOffset;
                        var characterRightX = itemX + this.character.spriteWidth + this.character.xOffset;
                        var characterTopY = itemY - this.character.yOffset + this.character.spriteHeight / 2; // let it overlap if it's halfway
                        var characterBottomY = itemY + this.character.spriteHeight;
                        // the character bottom one doesn't need an offset, just the top

                        if (characterRightX > blockLeftX && characterRightX < blockRightX || characterLeftX < blockRightX && characterLeftX > blockLeftX) {
                            if (characterBottomY > blockTopY && characterBottomY < blockBottomY || characterTopY < blockBottomY && characterTopY > blockTopY) {
                        ...
                        var bulletPosition = this.getRelativePosition(itemX, itemY);
                        if (bulletPosition[0] === Xi && bulletPosition[1] === Yi) {
                            return true;
                        }
                    } else if (thingToCheck === 'ray') {
                        // check bullet collision for all tiles
                        ...
                        var blob = item;
                        var blobLeftX = itemX + blob.leftEmptyPixels;
                        var blobRightX = itemX + blob.spriteWidth - blob.rightEmptyPixels;
                        var blobTopY = itemY + blob.topEmptyPixels;
                        var blobBottomY = itemY + blob.spriteHeight;

                        if (blobRightX > blockLeftX && blobRightX < blockRightX || blobLeftX < blockRightX && blobLeftX > blockLeftX) {
                            if (blobBottomY > blockTopY && blobBottomY < blockBottomY || blobTopY < blockBottomY && blobTopY > blockTopY) {
    ...
    checkDoorCollision: function checkDoorCollision() {
        var result = this.isDoorCollision();
    ...
    isDoorCollision: function isDoorCollision() {
        // check for both character collision and paint bullet collision -
        // store results in array. First item is character,second item is bullet.

        var characterX = this.character.x;
        var characterY = this.character.y;

        // store relevent door values
        var doorLeftX = this.door.x;
        var doorRightX = this.door.x + this.door.width;
        var doorTopY = this.door.y;
        var doorBottomY = this.door.y + this.door.height;

        // check character collision - only bother if collision hasn't been found
        var characterLeftX = characterX - this.character.xOffset;
        var characterRightX = characterX + this.character.spriteWidth + this.character.xOffset;
        var characterTopY = characterY - this.character.yOffset; // let it overlap if it's halfway
        var characterBottomY = characterY + this.character.spriteHeight;
        // the character bottom one doesn't need an offset, just the top

        if (characterRightX > doorLeftX && characterRightX < doorRightX || characterLeftX < doorRightX && characterLeftX > doorLeftX) {
            if (characterBottomY > doorTopY && characterBottomY < doorBottomY || characterTopY < doorBottomY && characterTopY > doorTopY) {
    ...
    changeSpriteState: function changeSpriteState(eventKey, action) {
        if (action === 'start') {

            // start with walking and standing
            if (eventKey == "ArrowUp") {
                this.character.currentState = this.character.walkBackward;
                this.character.currentSpeed = this.character.walkSpeed;
                ...
                this.character.currentState = this.character.walkLeft;
                this.character.currentSpeed = this.character.walkSpeed;
                this.character.direction = 'left';
            } else if (eventKey == "ArrowRight") {
                this.character.currentState = this.character.walkRight;
                this.character.currentSpeed = this.character.walkSpeed;
            ...
            //this.character.frameIndex = 1;
                ...
                // start with walking and standing
                if (eventKey == "ArrowUp") {
                    this.character.currentState = this.character.standBackward;
                    this.character.currentSpeed = 0;
                } else if (eventKey == "ArrowDown") {
                    this.character.currentState = this.character.standForward;
                    this.character.currentSpeed = 0;
                } else if (eventKey == "ArrowLeft") {
                    this.character.currentState = this.character.standLeft;
                    this.character.currentSpeed = 0;
                } else if (eventKey == "ArrowRight") {
                    this.character.currentState = this.character.standRight;
                    this.character.currentSpeed = 0;
                }
                this.character.currentStateFrame = this.character.currentState.frames[0];
    ...
    continueSpriteFrames: function continueSpriteFrames() {
        //this.character.lastFrameIndex = this.character.frameIndex;
        var animationLength = this.character.currentState.frames.length;
        var newIndex = this.character.currentStateFrame.index + 1;
        if (newIndex >= animationLength) {
            newIndex = 0;
        }

        this.character.currentStateFrame = this.character.currentState.frames[newIndex];
    ...
    moveCharacter: function moveCharacter() {
        ...
        if (this.character.currentSpeed === 0 || this.character.x === null || this.character.y === null) {
            // if it's 0 just skip it all
            return;
        }
        var newPosition = [this.character.x, this.character.y];
        if (this.character.direction === 'forward') {
            var newY = this.character.y + this.character.currentSpeed;
            if (newY < this.character.yOffset) {
                newY = this.character.yOffset;
            } else if (newY > this.canvas.height - this.character.spriteHeight) {
                newY = this.canvas.height - this.character.spriteHeight;
            }
            newPosition[1] = newY;
        } else if (this.character.direction === 'backward') {
            var newY = this.character.y - this.character.currentSpeed;
            ...
            var newX = this.character.x - this.character.currentSpeed;
            if (newX < this.character.xOffset) {
                newX = this.character.xOffset;
            } else if (newX > this.context.canvas.width - this.character.xOffset) {
                newX = this.context.canvas.width - this.character.xOffset;
            }
            newPosition[0] = newX;
        } else if (this.character.direction === 'right') {
            // let maxPosition = this.context.canvas.width - this.character.spriteWidth -
            // this.character.xOffset;
            var maxPosition = this.context.canvas.width - 46;
            // let maxPosition = 223;
            var newX = this.character.x + this.character.walkSpeed;
            if (newX < this.character.xOffset) {
                newX = this.character.xOffset;
        ...
        var isNpcCollision = false;
        for (var i = 0; i < this.npcs.length; i++) {
            isNpcCollision = this.isNpcCharacterCollision(this.npcs[i], newPosition[0], newPosition[1]);
            if (isNpcCollision) {
                break;
            }
        }
        if (!this.isBlockCollision('character', newPosition[0], newPosition[1], this.character) && !isNpcCollision) {
            this.character.x = newPosition[0];
            this.character.y = newPosition[1];
            this.setCharacterRelativePositions();
        }

        this.continueSpriteFrames();
    },

    // fill the entire board with random blocks
    fillWithBlocks: function fillWithBlocks() {
        // test
        ...
        for (var Xi = 0; Xi < this.level.columns; Xi++) {
            for (var Yi = this.spaceAtEdges; Yi < rows - this.spaceAtEdges; Yi++) {
    ...
    generateRandomBlock: function generateRandomBlock(Xi, Yi) {
        ...
        do {
            ...
            var possibleColors = getTileColors(this.level);
            var chosenNumber = Math.floor(Math.random() * possibleColors.length);
            var chosenColor = possibleColors[chosenNumber];
    ...
    isBlockValid: function isBlockValid(newBlock) {
        var blockInRowColorCount = this.countBlocksInRow(newBlock, null);
    ...
    countBlocksInRow: function countBlocksInRow(newBlock, blocksInRow) {
        // uses recursion

        // count possible blocks around it in the same color
        var color = newBlock.color;
        var totalCount = 0;
        var coordinates = [[newBlock.x, newBlock.y - 1], [newBlock.x + 1, newBlock.y], [newBlock.x, newBlock.y + 1], [newBlock.x - 1, newBlock.y]];
        ...
        for (var i = 0; i < coordinates.length; i++) {
            ...
            if (coordinates[i][0] >= 0 && coordinates[i][0] < this.level.columns && coordinates[i][1] >= 0 && coordinates[i][1] < rows) {
                ...
                if (this.blocks[coordinates[i][0]][coordinates[i][1]] != null) {
                    var compareBlock = this.blocks[coordinates[i][0]][coordinates[i][1]];
                    ...
                    if (compareBlock.color === color && !this.isContainedInBlockList(compareBlock, blocksInRow)) {
                        // add block to list
                        blocksInRow.push(compareBlock);
                        // add to count
                        totalCount++;
                        // get count for that block just compared - and then add to total
                        var nextCount = this.countBlocksInRow(compareBlock, blocksInRow);
    ...
    destroyBlocks: function destroyBlocks(changedBlock) {
        // get list of blocks that are the same color starting with this block
        var blockGroup = [changedBlock];
        ...
        var neededCount = this.level.blocksForScore;
    ...
    clearBlock: function clearBlock(block) {
    ...
    getBlocksInRow: function getBlocksInRow(newBlock, blocksInRow) {
        // uses recursion
            ...
            if (coordinates[i][0] >= 0 && coordinates[i][0] < this.level.columns && coordinates[i][1] >= 0 && coordinates[i][1] < rows) {
                ...
                if (this.blocks[coordinates[i][0]][coordinates[i][1]] != null) {
                    ...
                    if (compareBlock.color === color && !this.isContainedInBlockList(compareBlock, blocksInRow)) {
                        ...
                        totalCount++;
                        // get count for that block just compared - and then add to total
                        var nextCount = this.getBlocksInRow(compareBlock, blocksInRow);
                        totalCount += nextCount;
                    }
                }
            }
    ...
    isContainedInBlockList: function isContainedInBlockList(block, list) {
    ...
    createBlock: function createBlock(color, Xi, Yi) {

        // create block
        var newBlock = new Block(this.tileWidth, this.tileHeight, Xi, Yi, color);
    ...
    drawBlock: function drawBlock(block) {
        var x = block.x * this.tileWidth;
        var y = block.y * this.tileHeight;
        ...
        this.context.rect(x + 2, y + 2, this.tileWidth - 4, this.tileHeight - 4);
        this.context.stroke();
    ...
    redrawBlocks: function redrawBlocks() {
    ...
    setCanvas: function setCanvas() {
        ...
        var columns = this.level.columns;
        var rows = this.level.rows;
        if (rows < 14) {
            // change dimensions for display reasons
            rows = 14;
            this.spaceAtEdges = 4;
        } else {
            this.spaceAtEdges = 3;
    ...
    setCharacterRelativePositions: function setCharacterRelativePositions() {
        // TODO test that this works right
        var x = this.character.x + this.character.spriteHeight / 2;
        var y = this.character.y + this.character.spriteHeight / 2;
        var divideX = Math.ceil(x / this.tileWidth) - 1;
        var divideY = Math.ceil(y / this.tileHeight) - 1;
        this.character.Xi = divideX;
        this.character.Yi = divideY;
    },

    getRelativePosition: function getRelativePosition(x, y) {
        var Xi = Math.ceil(x / this.tileWidth) - 1;
        var Yi = Math.ceil(y / this.tileHeight) - 1;
        return [Xi, Yi];
    },
    ...
    drawCharacter: function drawCharacter() {
        ...
        var sx = this.character.currentStateFrame.startX;
        var sy = this.character.currentStateFrame.startY;

        var sw = this.character.currentStateFrame.width;
        var sh = this.character.currentStateFrame.height;

        var offX = Math.round((this.character.currentStateFrame.width - this.tileWidth) / 2);
        var offY = Math.round((this.character.currentStateFrame.height - this.tileHeight) / 2);
        ...
        var image = this.character.currentStateFrame.image;
        var isLoaded = image.complete && image.naturalHeight !== 0;
        if (!isLoaded) {
            image.addEventListener('load', function () {
                Game.context.drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh);
    ...
    drawNpcs: function drawNpcs(npcs) {
        for (var i = 0; i < npcs.length; i++) {
            this.drawNpc(npcs[i]);
        }
    },

    drawNpc: function drawNpc(npc) {
        ...
        var sx = npc.currentStateFrame.startX;
        var sy = npc.currentStateFrame.startY;

        var sw = npc.currentStateFrame.width;
        var sh = npc.currentStateFrame.height;

        var offX = Math.round((npc.currentStateFrame.width - this.tileWidth) / 2);
        var offY = Math.round((npc.currentStateFrame.height - this.tileHeight) / 2);
        ...
        var image = npc.currentStateFrame.image;
        var isLoaded = image.complete && image.naturalHeight !== 0;
        if (!isLoaded) {
            image.addEventListener('load', function () {
                Game.context.drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh);
    ...
    drawGrid: function drawGrid() {
        ...
        for (var Xi = 1; Xi < this.level.columns; Xi++) {
            var x = Xi * this.tileWidth;
            var yStart = 0;
            var yEnd = this.context.canvas.height;
        ...
        for (var Yi = 1; Yi < rows; Yi++) {
            var y = Yi * this.tileWidth;
            var xStart = 0;
            var xEnd = this.context.canvas.width;
    ...
    setGameArea: function setGameArea() {
        var gameArea = document.getElementById('gameArea');
        var newWidth = this.context.canvas.width + 275;
        gameArea.style.width = newWidth + 'px';
        var height = this.context.canvas.height + 60;
        gameArea.style.height = height + 'px';
    },

    addBlobToScore: function addBlobToScore(blob) {
        var multiplier = 3;
        var blobValue = 0;
        var blobColor = blob.color;
        if (blobColor === 'white') {
            console.log('white');
    ...
    addBlocksToScore: function addBlocksToScore(blocks) {
        // figure out the initial value of one block
        var blockValue = 0;
        var scoreToAdd = 0;
        var blockColor = blocks[0].color;
    ...
    updateScoreText: function updateScoreText() {
        this.scoreText.innerHTML = this.score + this.levelScore;
    },

    updateLevelText: function updateLevelText() {
        this.levelText.innerHTML = this.levelNumber;
    },

    isPrimary: function isPrimary(color) {
    ...
    isSecondary: function isSecondary(color) {
    ...
    createDoor: function createDoor() {
        // create new door
        this.door = new Door(this.doorColor, this.tileWidth, this.tileHeight, this.context.canvas.width, this.context.canvas.height);
    },

    drawDoor: function drawDoor() {
        this.context.beginPath();
        this.context.fillStyle = this.door.doorHex;
        this.context.fillRect(this.door.x, this.door.y, this.door.width, this.door.height);
    },

    showNextInScene: function showNextInScene() {
        //console.log('next in scene');
        if (this.currentDialogue === null && this.currentAnimationInterval === null) {
            this.inScene = false;
            ...
            if (this.currentAnimationInterval != null && !this.currentAnimationInterval.complete) {
                ...
                if (!this.currentDialogue.complete) {
                    this.allowDialogue = true;
                    this.showDialogue();
                } else {
    ...
    getNextDialogue: function getNextDialogue() {
        this.currentDialogue = this.currentConversation.getNextDialogue();
        if (this.currentDialogue === null) {
            this.levelStarted = true;
            this.currentConversation = null;
    ...
    setConversation: function setConversation(conversationType) {
    ...
    setAnimationInterval: function setAnimationInterval(animationIndex) {
        this.currentAnimationInterval = this.level.openingScene.animation.animations[animationIndex];
    },

    performAnimationInterval: function performAnimationInterval() {
        this.currentAnimationInterval.continueAnimation(this);
    },

    setOpeningScene: function setOpeningScene() {
    ...
    testStartConversation: function testStartConversation() {
        this.currentConversation = testConversation;
        this.currentDialogue = this.currentConversation.getDialogue();
    },

    testShowDialogue: function testShowDialogue() {
        this.currentDialogue = testDialogue;
    },

    testHideDialogue: function testHideDialogue() {
        this.currentDialogue = null;
    },

    testShowMood: function testShowMood(character, dx, dy) {
        if (character.currentMood === null) {
            return;
        }

        var sx = character.currentMood.visual.startX;
        var sy = character.currentMood.visual.startY;

        var sw = character.currentMood.visual.width;
        var sh = character.currentMood.visual.height;

        var dw = sw;
        var dh = sh;

        var image = character.currentMood.visual.image;
        var isLoaded = image.complete && image.naturalHeight !== 0;
        if (!isLoaded) {
            image.addEventListener('load', function () {
                Game.context.drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh);
    ...
    skipToDoor: function skipToDoor() {
        var newY = this.context.canvas.height - this.tileHeight * 3;
        this.character.y = newY;
    },

    goToLevel: function goToLevel(levelNumber) {
