...
Game.getNextDialogue();}else if(instructionsModal.style.display != "none"){ // this should only do something if a modal is displayed
...
if(!Game.showInventoryMenu && !Game.inScene){Game.showInventoryMenu = true;Game.levelStarted = false;}else if(Game.showInventoryMenu && !Game.inScene){Game.showInventoryMenu = false;Game.levelStarted = true;}}else if(event.key === 'ArrowUp' || event.key === 'ArrowDown' || event.key === 'ArrowLeft' || event.key === 'ArrowRight'){if(Game.disallowMovement){return;}Game.changeSpriteState(event.key,'start');}else if(event.key === ' ' || event.key === 'Spacebar'){ // do something different if the inventory menu is open
if(Game.showInventoryMenu){ // if dialogue is happening, go to next dialogue
}else if(Game.currentDialogue != null && Game.allowDialogue){ //Game.endCurrentDialogue();
Game.getNextDialogue();}else { // determine if they're shooting or absorbing
...
if(this.showInventoryMenu){}else {Game.inventory.selectVial('up');playSound('select vial');Game.showInventory();}}else if(event.key === 'z' || event.key === 'Z'){ // check if the inventory is open or not
if(this.showInventoryMenu){}else {Game.inventory.selectVial('down');playSound('select vial');Game.showInventory();}}else if(event.key === "q" || event.key === "Q"){ // check if the inventory is open or not
if(this.showInventoryMenu){}else {Game.inventory.selectAbsorbColor('up',Game.levelStarted);}}else if(event.key === 'w' || event.key === 'W'){ // check if the inventory is open or not
if(this.showInventoryMenu){}else {Game.inventory.selectAbsorbColor('down',Game.levelStarted);}}},keyUpListener:function keyUpListener(event){Game.keyPresses[event.key] = false; //console.log('Key up: ' + event.key);
// determine action
...
//Game.testDrawPipe();
Game.drawBlobsAndCharacter();Game.drawBullets();Game.moveBullets();Game.drawRay();Game.drawDoor();Game.checkBlobCharacterCollision();Game.checkDoorCollision();if(Game.level.isSpecialLevel){if(Game.inScene && Game.gameModeChosen & Game.newOrLoadChosen && Game.showOpeningScene){Game.showNextInScene();}else {Game.showDialogue();if(Game.currentConversation != null && Game.currentConversation.type === 'exit'){if(Game.currentDialogue != null && !Game.currentDialogue.complete){Game.allowDialogue = true;}else if(Game.currentAnimationInterval != null){Game.allowDialogue = false;Game.performExitAnimation();}} // check inscene and showopening just in case
if(!Game.showOpeningScene && Game.inScene === true){Game.inScene = false;}if(Game.showInventoryMenu){Game.inventory.menu.drawMenu(Game.context);}}}}window.requestAnimationFrame(Game.gameLoop);},generateBlobs:function generateBlobs(){ // clear blobs
...
this.door = new Door(this.doorColor,this.tileWidth,this.tileHeight,this.context.canvas.width,this.context.canvas.height);},drawDoor:function drawDoor(){this.context.beginPath();this.context.fillStyle = this.door.doorHex;this.context.fillRect(this.door.x,this.door.y,this.door.width,this.door.height);},showNextInScene:function showNextInScene(){ //if (this.openingScene.actionOrder[this.openingScene.actionIndex] === 'D' &&
...
//        }
//    }
if(this.openingScene === null){return;}if(this.openingScene.animation != null && this.openingScene.actionOrder[this.openingScene.actionIndex] === 'A'){ // loop through the number of animations to do at once
...
//this.openingScene.actionIndex++;
if(this.openingScene.animationIndex >= this.openingScene.animation.animations.length){this.openingScene.animationIndex = 0;}if(this.openingScene === null){this.currentAnimationInterval = null;}else {this.currentAnimationInterval = this.openingScene.animation.getNextInterval();this.allowDialogue = true;}}}if(allComplete){this.openingScene.animationIndex += animationsAtOnce;this.openingScene.actionIndex += animationsAtOnce;}}else {if(this.currentDialogue != null && !this.currentDialogue.complete){this.allowDialogue = true;this.showDialogue();} //else if (this.openingScene.conversation.dialogues[this.openingScene.conversationIndex].complete && 
...
this.currentConversation = null;}else {this.currentDialogue = this.currentConversation.getNextDialogue();}}}if((this.openingScene.animation === null || this.openingScene.animation.animations === null || this.openingScene.animation.animations.length === 0 || this.openingScene.animation.animations[this.openingScene.animation.animations.length - 1].complete) && (this.openingScene.conversation === null || this.openingScene.conversation.dialogues === null || this.openingScene.conversation.dialogues.length === 0 || this.openingScene.conversation.dialogues[this.openingScene.conversation.dialogues.length - 1].complete)){this.levelStarted = true;this.inScene = false;this.openingScene = null;this.showOpeningScene = false;} //if (this.currentAnimationInterval != null && !this.currentAnimationInterval.complete) {
...
restartLevel:function restartLevel(){if(this.inScene != true){this.goToLevel(this.levelNumber,false);}}};
